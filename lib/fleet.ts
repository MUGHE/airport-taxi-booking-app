...
export const DEFAULT_CONGESTION_ZONE: [number, number][] = [
  ...
  [51.5323, -0.1058], // Angel
  [51.5257, -0.0876], // Old Street
  [51.5165, -0.0730], // Whitechapel / Aldgate
  [51.5079, -0.0754], // Tower Bridge (north)
  [51.5031, -0.0754], // Tower Bridge (south)
  [51.4948, -0.1006], // Elephant & Castle
  [51.4861, -0.1229], // Vauxhall
  [51.4965, -0.1447], // Victoria
  [51.5027, -0.1527], // Hyde Park Corner
  [51.5131, -0.1590], // Marble Arch
]

// Ray casting: count polygon edges crossed by a ray going east from the point.
function inPolygon(lat: number, lng: number, polygon: [number, number][]): boolean {
  let inside = false
  for (let i = 0, j = polygon.length - 1; i < polygon.length; j = i++) {
    const [lat1, lng1] = polygon[i]
    const [lat2, lng2] = polygon[j]
    if (lat1 > lat !== lat2 > lat && lng < ((lng2 - lng1) * (lat - lat1)) / (lat2 - lat1) + lng1) inside = !inside
  }
  return inside
}
