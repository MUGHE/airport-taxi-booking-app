...
import type { RelatedDestinationCandidate } from "@/lib/related-destinations"
import { geographicDistanceKm, orderNearbyPlaceCandidates } from "@/lib/place-page-rules"
...
function PlaceRelationshipsEditor({ form, candidates, onAdd, onUpdate }: { form: SaveAdminDestinationPageInput; candidates: RelatedDestinationCandidate[]; onAdd: (pageId: string, kind: "supported_airport" | "nearby_place") => void; onUpdate: (relationships: AdminRelatedDestination[]) => void }) {
  const selected = new Set(form.relatedDestinations.map((item) => item.pageId))
  const airports = candidates.filter((item) => item.pageType === "airport" && !selected.has(item.id))
  const places = orderNearbyPlaceCandidates(form.primaryParentId, candidates.filter((item) => item.pageType === "place" && !selected.has(item.id)).map((item) => ({ ...item, distanceKm: item.latitude == null || item.longitude == null || !form.latitude || !form.longitude ? null : geographicDistanceKm(form.latitude, form.longitude, item.latitude, item.longitude) })))
  function move(index: number, direction: -1 | 1) {
    const relationship = form.relatedDestinations[index]
    const sameKindIndexes = form.relatedDestinations.flatMap((item, itemIndex) => item.kind === relationship.kind ? [itemIndex] : [])
    const position = sameKindIndexes.indexOf(index)
    const target = sameKindIndexes[position + direction]
    if (target == null) return
    const next = [...form.relatedDestinations]
    ;[next[index], next[target]] = [next[target], next[index]]
    onUpdate(next.map((item, itemIndex) => ({ ...item, sortOrder: itemIndex })))
  }
  return <div className="space-y-6">
    {(["supported_airport", "nearby_place"] as const).map((kind) => {
      const items = form.relatedDestinations.filter((item) => item.kind === kind)
      const options = kind === "supported_airport" ? airports : places
      return <section key={kind} className="space-y-3">
        <div><h4 className="font-medium">{kind === "supported_airport" ? "Supported Airports" : "Nearby Places"}</h4><p className="text-sm text-muted-foreground">{kind === "supported_airport" ? "Published airports only. Unavailable airports stay listed but cannot be booked." : "Choose three to six when enough Published Place Pages exist. Same-parent suggestions appear first."}</p></div>
        <select aria-label={`Add ${kind === "supported_airport" ? "Supported Airport" : "Nearby Place"}`} className="h-9 w-full rounded-lg border border-input bg-background px-2 text-sm" defaultValue="" onChange={(event) => { onAdd(event.target.value, kind); event.target.value = "" }}><option value="">Add {kind === "supported_airport" ? "a Published Airport Page" : "a Published Place Page"}</option>{options.map((candidate) => <option key={candidate.id} value={candidate.id}>{candidate.displayName}{kind === "supported_airport" && !candidate.bookingAvailable ? " · Booking temporarily unavailable" : ""}</option>)}</select>
        <div className="space-y-3">{items.map((item, position) => { const index = form.relatedDestinations.findIndex((relationship) => relationship.pageId === item.pageId); return <div key={item.pageId} className="rounded-lg border border-border p-3"><div className="flex items-center justify-between gap-2"><div><p className="font-medium">{item.displayName}</p><p className="text-xs text-muted-foreground">{!item.bookingAvailable && kind === "supported_airport" ? "Booking temporarily unavailable · remains visible" : item.slug}</p></div><div className="flex gap-1"><Button type="button" variant="ghost" size="icon-xs" aria-label={`Move ${item.displayName} up`} disabled={position === 0} onClick={() => move(index, -1)}><ArrowUp /></Button><Button type="button" variant="ghost" size="icon-xs" aria-label={`Move ${item.displayName} down`} disabled={position === items.length - 1} onClick={() => move(index, 1)}><ArrowDown /></Button><Button type="button" variant="ghost" size="icon-xs" aria-label={`Remove ${item.displayName}`} onClick={() => onUpdate(form.relatedDestinations.filter((relationship) => relationship.pageId !== item.pageId))}><Trash2 /></Button></div></div>{kind === "supported_airport" && <div className="mt-3 grid gap-3 sm:grid-cols-2"><div className="space-y-1"><Label>To airport heading (optional)</Label><Input value={item.heading} onChange={(event) => onUpdate(form.relatedDestinations.map((relationship) => relationship.pageId === item.pageId ? { ...relationship, heading: event.target.value } : relationship))} /></div><div className="space-y-1"><Label>To airport description (optional)</Label><textarea className="min-h-20 w-full rounded-lg border border-input px-2.5 py-2 text-sm" value={item.description} onChange={(event) => onUpdate(form.relatedDestinations.map((relationship) => relationship.pageId === item.pageId ? { ...relationship, description: event.target.value } : relationship))} /></div><div className="space-y-1"><Label>From airport heading (optional)</Label><Input value={item.reverseHeading} onChange={(event) => onUpdate(form.relatedDestinations.map((relationship) => relationship.pageId === item.pageId ? { ...relationship, reverseHeading: event.target.value } : relationship))} /></div><div className="space-y-1"><Label>From airport description (optional)</Label><textarea className="min-h-20 w-full rounded-lg border border-input px-2.5 py-2 text-sm" value={item.reverseDescription} onChange={(event) => onUpdate(form.relatedDestinations.map((relationship) => relationship.pageId === item.pageId ? { ...relationship, reverseDescription: event.target.value } : relationship))} /></div></div>}</div> })}</div>
      </section>
    })}
  </div>
}
...
export function DestinationPageEditor({ pageType, initialPage, relatedCandidates, reusableContent, placeIdentityOptions }: { pageType: DestinationPageType; initialPage?: AdminDestinationPage; relatedCandidates: RelatedDestinationCandidate[]; reusableContent: ReusableDestinationContent; placeIdentityOptions: { groups: AdminPlaceGroup[]; parents: AdminParentPlace[] } }) {
  ...
  const [replacementSlug, setReplacementSlug] = useState(pageType === "place" ? "destinations" : "airport-transfers")
  ...
  const placeOptionalSectionTypes = new Set<DestinationSectionType>(["map", "related_destinations", "city_routes", "ferry_cruise", "video"])
  const availableSectionTypes = DESTINATION_SECTION_TYPES.filter((type) => pageType === "airport" || getDestinationPagePolicy("place").content.requiredSectionTypes.includes(type) || placeOptionalSectionTypes.has(type))
  ...
  function updateLocalFaq(index: number, field: "question" | "answer", value: string) {
    const key = pageType === "place" ? "placeFaqs" : "airportFaqs"
    updateContent({ ...content, [key]: content[key].map((faq, faqIndex) => faqIndex === index ? { ...faq, [field]: value } : faq) })
  }
  const localFaqs = content.placeFaqs
  const minimumLocalFaqs = 2
  ...
  function addSection(type: DestinationSectionType) {
    const section = createDestinationSection(type, `${type}-${Date.now()}`, pageType)
  ...
  function addRelatedDestination(pageId: string) {
    ...
    update("relatedDestinations", [...form.relatedDestinations, { pageId, displayName: candidate.displayName, slug: candidate.slug, heading: `Travel between ${form.displayName || "this airport"} and ${candidate.displayName}`, description: `Book a fixed-price airport transfer between ${form.displayName || "this airport"} and ${candidate.displayName}.`, reverseHeading: `Travel between ${candidate.displayName} and ${form.displayName || "this airport"}`, reverseDescription: `Book a fixed-price airport transfer between ${candidate.displayName} and ${form.displayName || "this airport"}.`, kind: "related_route", sortOrder: form.relatedDestinations.length, bookingAvailable: candidate.bookingAvailable }])
  }
  function addPlaceRelationship(pageId: string, kind: "supported_airport" | "nearby_place") {
    const candidate = relatedCandidates.find((item) => item.id === pageId)
    if (!candidate || form.relatedDestinations.some((item) => item.pageId === pageId)) return
    const next = [...form.relatedDestinations, { pageId, displayName: candidate.displayName, slug: candidate.slug, heading: "", description: "", reverseHeading: "", reverseDescription: "", kind, sortOrder: form.relatedDestinations.filter((item) => item.kind === kind).length, bookingAvailable: candidate.bookingAvailable }]
    updatePlaceRelationships(next)
  }
  function updatePlaceRelationships(next: AdminRelatedDestination[]) {
    const supportedNames = (items: AdminRelatedDestination[]) => items.filter((item) => item.kind === "supported_airport").map((item) => item.displayName)
    const generatedDescription = policy.defaults.metaDescription(form.displayName, supportedNames(form.relatedDestinations))
    update("relatedDestinations", next)
    if (form.metaDescription === generatedDescription) update("metaDescription", policy.defaults.metaDescription(form.displayName, supportedNames(next)))
  }
        ...
        <Tabs value={activeTab} onValueChange={(value) => setActiveTab(value as EditorTab)}>
          ...
          <div className="mt-6">
                ...
                <div className="grid items-start gap-5 lg:grid-cols-[minmax(240px,0.75fr)_minmax(0,1.6fr)]">
                  ...
                  <div className="rounded-xl border border-border bg-background p-4 sm:p-5">
                    {selectedContentId === "final-cta" ? <div className="space-y-4"><div><h4 className="font-semibold">Final booking CTA</h4><p className="text-sm text-muted-foreground">The closing booking prompt shown after page content.</p></div><div className="space-y-1.5"><Label htmlFor="final-cta-heading">Heading</Label><Input id="final-cta-heading" value={content.finalCta.heading} onChange={(event) => updateContent({ ...content, finalCta: { ...content.finalCta, heading: event.target.value } })} /></div><div className="space-y-1.5"><Label>Description</Label><RichTextEditor value={content.finalCta.bodyDocument ?? content.finalCta.body} onChange={(bodyDocument) => updateContent({ ...content, finalCta: { ...content.finalCta, body: tiptapDocumentToBlocks(bodyDocument), bodyDocument } })} /></div></div> : selectedContentSection ? <div className="space-y-4">
                      ...
                      {pageType === "place" && <div className="space-y-3 border-t border-border pt-4"><div className="flex items-center justify-between gap-3"><div><Label>Private Source Notes</Label><p className="text-xs text-muted-foreground">Record sources for figures, restrictions, and operating rules. These notes never appear publicly.</p></div><Button type="button" variant="outline" size="sm" onClick={() => { const index = content.sections.findIndex((section) => section.id === selectedContentSection.id); updateSection(index, { sourceNotes: [...selectedContentSection.sourceNotes, { sourceName: "", sourceUrl: "", checkedDate: "" }] }) }}><Plus className="size-4" /> Add source</Button></div>{selectedContentSection.sourceNotes.map((note, noteIndex) => <div key={noteIndex} className="grid gap-2 rounded-lg border border-border p-3 sm:grid-cols-[1fr_1fr_160px_auto]"><Input aria-label={`Source ${noteIndex + 1} name`} placeholder="Source name" value={note.sourceName} onChange={(event) => { const index = content.sections.findIndex((section) => section.id === selectedContentSection.id); updateSection(index, { sourceNotes: selectedContentSection.sourceNotes.map((item, itemIndex) => itemIndex === noteIndex ? { ...item, sourceName: event.target.value } : item) }) }} /><Input aria-label={`Source ${noteIndex + 1} URL`} type="url" placeholder="https://…" value={note.sourceUrl} onChange={(event) => { const index = content.sections.findIndex((section) => section.id === selectedContentSection.id); updateSection(index, { sourceNotes: selectedContentSection.sourceNotes.map((item, itemIndex) => itemIndex === noteIndex ? { ...item, sourceUrl: event.target.value } : item) }) }} /><Input aria-label={`Source ${noteIndex + 1} checked date`} type="date" value={note.checkedDate} onChange={(event) => { const index = content.sections.findIndex((section) => section.id === selectedContentSection.id); updateSection(index, { sourceNotes: selectedContentSection.sourceNotes.map((item, itemIndex) => itemIndex === noteIndex ? { ...item, checkedDate: event.target.value } : item) }) }} /><Button type="button" variant="ghost" size="icon-xs" aria-label={`Remove source ${noteIndex + 1}`} onClick={() => { const index = content.sections.findIndex((section) => section.id === selectedContentSection.id); updateSection(index, { sourceNotes: selectedContentSection.sourceNotes.filter((_, itemIndex) => itemIndex !== noteIndex) }) }}><Trash2 /></Button></div>)}</div>}
            ...
            <TabsContent value="faq-trust">
              <EditorPanel title="FAQs and trust content" description={pageType === "airport" ? "Airport FAQs are managed centrally for every airport page." : "Add place-specific answers and choose approved shared content."}>
                {pageType === "airport" ? <div className="rounded-lg border border-dashed border-border bg-secondary/40 p-4 text-sm text-muted-foreground">This airport uses the central Airport FAQs managed from the Admin dashboard. Page-specific FAQ editing is disabled here.</div> : <>
                  <div><h4 className="font-medium">Place-specific FAQs</h4><div className="mt-1 flex items-center justify-between gap-3"><p className="text-sm text-muted-foreground">Add at least {minimumLocalFaqs} complete local questions.</p><span className={completedLocalFaqCount < minimumLocalFaqs ? "text-sm text-destructive" : "text-sm text-emerald-700"}>{completedLocalFaqCount}/{minimumLocalFaqs} minimum</span></div><div className="mt-3 space-y-3">{localFaqs.map((faq, index) => <div key={faq.id} className="rounded-lg border border-border p-3"><div className="grid gap-3"><Input aria-label={`Local FAQ ${index + 1} question`} value={faq.question} onChange={(event) => updateLocalFaq(index, "question", event.target.value)} placeholder="Local question" /><textarea aria-label={`Local FAQ ${index + 1} answer`} className="min-h-20 w-full rounded-lg border border-input px-2.5 py-2 text-sm" value={faq.answer} onChange={(event) => updateLocalFaq(index, "answer", event.target.value)} placeholder="Local answer" /></div></div>)}</div><Button type="button" variant="outline" size="sm" className="mt-3" onClick={() => updateContent({ ...content, placeFaqs: [...content.placeFaqs, { id: `local-faq-${Date.now()}`, question: "", answer: "" }] })}><Plus className="size-4" /> Add local FAQ</Button></div>
              ...
              <EditorPanel title={pageType === "place" ? "Supported Airports and Nearby Places" : "Related Routes"} description={pageType === "place" ? "Connect only Published Pages. Temporarily unavailable airports remain visible with an explanation." : "Connect this Airport Page to another Published Airport Page. Each direction keeps its own wording and image."}>
                {pageType === "place" ? <PlaceRelationshipsEditor form={form} candidates={relatedCandidates} onAdd={addPlaceRelationship} onUpdate={updatePlaceRelationships} /> : <>
                {!form.id && <p className="rounded-lg bg-secondary px-3 py-2 text-sm">Save this page as a Draft first, then add Related Routes.</p>}
                {form.id && <select aria-label="Published Related Route" className="h-9 w-full rounded-lg border border-input bg-background px-2 text-sm" defaultValue="" onChange={(event) => { addRelatedDestination(event.target.value); event.target.value = "" }}><option value="">Add a Published Airport Page</option>{relatedCandidates.filter((candidate) => candidate.pageType === "airport" && !form.relatedDestinations.some((item) => item.pageId === candidate.id)).map((candidate) => <option key={candidate.id} value={candidate.id}>{candidate.displayName} · {candidate.slug}</option>)}</select>}
