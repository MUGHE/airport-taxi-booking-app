          ...
          <div className="mt-6">
            ...
            <TabsContent value="faq-trust">
              <EditorPanel title="FAQs and trust content" description="Choose approved shared content and add airport-specific answers.">
                <div><h4 className="font-medium">{pageType === "place" ? "Place-specific" : "Airport-specific"} FAQs</h4><div className="mt-1 flex items-center justify-between gap-3"><p className="text-sm text-muted-foreground">Add at least {minimumLocalFaqs} complete local questions.</p><span className={completedLocalFaqCount < minimumLocalFaqs ? "text-sm text-destructive" : "text-sm text-emerald-700"}>{completedLocalFaqCount}/{minimumLocalFaqs} minimum</span></div><div className="mt-3 space-y-3">{localFaqs.map((faq, index) => <div key={faq.id} className="rounded-lg border border-border p-3"><div className="grid gap-3"><Input aria-label={`Local FAQ ${index + 1} question`} value={faq.question} onChange={(event) => updateLocalFaq(index, "question", event.target.value)} placeholder="Local question" /><textarea aria-label={`Local FAQ ${index + 1} answer`} className="min-h-20 w-full rounded-lg border border-input px-2.5 py-2 text-sm" value={faq.answer} onChange={(event) => updateLocalFaq(index, "answer", event.target.value)} placeholder="Local answer" /></div></div>)}</div><Button type="button" variant="outline" size="sm" className="mt-3" onClick={() => { const key = pageType === "place" ? "placeFaqs" : "airportFaqs"; updateContent({ ...content, [key]: [...content[key], { id: `local-faq-${Date.now()}`, question: "", answer: "" }] }) }}><Plus className="size-4" /> Add local FAQ</Button></div>
