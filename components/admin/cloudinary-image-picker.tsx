...
export function CloudinaryImagePicker({ kind, value, onChange }: { kind: CloudinaryImageKind; value?: DestinationImageReference; onChange: (value?: DestinationImageReference) => void }) {
  ...
  return <div className="space-y-3 rounded-lg border border-dashed border-border p-4">
    ...
    {value && <div className="flex items-center gap-3 rounded-md bg-secondary/60 p-2 text-sm"><ProgressiveImage wrapperClassName="size-14 shrink-0 rounded bg-muted" src={value.secureUrl} alt={value.altText} sizes="56px" placeholder="blur" blurDataURL={cloudinaryBlurUrl(value.secureUrl)} className="object-contain" /><span className="min-w-0 flex-1 truncate">{value.publicId}</span><Button type="button" variant="ghost" size="sm" onClick={() => onChange(undefined)}>Remove</Button></div>}
