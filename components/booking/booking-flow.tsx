...
import { ArrowLeft, ArrowRight, Briefcase, Check, Loader2, MapPin, Users } from "lucide-react"
import { Button } from "@/components/ui/button"
import { Input } from "@/components/ui/input"
import { Label } from "@/components/ui/label"
import { Textarea } from "@/components/ui/textarea"
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from "@/components/ui/select"
import { computeFare, formatCurrency } from "@/lib/fleet"
...
import type { BookingAddOn, VehicleClass } from "@/lib/types"
...
export function BookingFlow({ vehicles = [], addOns = [] }: { vehicles: VehicleClass[]; addOns: BookingAddOn[] }) {
  ...
  return <div className="grid gap-8 lg:grid-cols-[1fr_340px]"><div><Stepper step={step} /><div className="mt-8">
    ...
    {step === 1 && <VehicleStep vehicleId={vehicleId} setVehicleId={setVehicleId} distanceMiles={distanceMiles} durationMinutes={durationMinutes} vehicles={vehicles} loading={distanceLoading} />}
    ...
    <Summary pickup={pickup} dropoff={dropoff} quote={quote} addOnsTotal={addOnsTotal} vehicle={vehicle} pickupDate={pickupDate} pickupTime={pickupTime} /></div>
}
...
function TripStep({ pickup, setPickup, dropoff, setDropoff, pickupDate, setPickupDate, pickupTime, setPickupTime, today }: { pickup: PlaceSelection | null; setPickup: (value: PlaceSelection | null) => void; dropoff: PlaceSelection | null; setDropoff: (value: PlaceSelection | null) => void; pickupDate: string; setPickupDate: (value: string) => void; pickupTime: string; setPickupTime: (value: string) => void; today: string }) { return <div><Heading title="Plan your trip" desc="Choose your pickup and drop-off locations from Google Maps." /><div className="grid gap-4 sm:grid-cols-2"><Field label="Pickup location"><DestinationPicker defaultValue={pickup?.address} placeholder="Start typing a pickup address" onSelect={setPickup} onClear={() => setPickup(null)} /></Field><Field label="Drop-off location"><DestinationPicker defaultValue={dropoff?.address} placeholder="Start typing a drop-off address" onSelect={setDropoff} onClear={() => setDropoff(null)} /></Field><Field label="Pickup date"><Input type="date" min={today} value={pickupDate} onChange={(e) => setPickupDate(e.target.value)} /></Field><Field label="Pickup time"><Input type="time" value={pickupTime} onChange={(e) => setPickupTime(e.target.value)} /></Field></div></div> }
function VehicleStep({ vehicleId, setVehicleId, distanceMiles, durationMinutes, vehicles, loading }: { vehicleId: string; setVehicleId: (value: string) => void; distanceMiles: number | null; durationMinutes: number | null; vehicles: VehicleClass[]; loading: boolean }) { return <div><Heading title="Pick your vehicle" desc={loading ? "Calculating your route…" : "Fares are fixed and include all taxes, tolls, and gratuity."} /><div className="grid gap-4 sm:grid-cols-2">{vehicles.map((item) => { const quote = distanceMiles != null && durationMinutes != null ? computeFare(item, distanceMiles, durationMinutes) : null; const selected = item.id === vehicleId; return <button key={item.id} type="button" onClick={() => setVehicleId(item.id)} className={cn("flex gap-4 rounded-2xl border p-4 text-left", selected ? "border-primary bg-primary/5 ring-1 ring-primary" : "border-border bg-card")}><div className="relative size-24 shrink-0 overflow-hidden rounded-xl bg-secondary"><Image src={item.image || "/placeholder.svg"} alt={item.name} fill className="object-cover" sizes="96px" /></div><div><h3 className="font-semibold">{item.name}</h3><div className="mt-1 flex gap-3 text-xs text-muted-foreground"><span className="flex items-center gap-1"><Users className="size-3.5" />{item.capacity}</span><span className="flex items-center gap-1"><Briefcase className="size-3.5" />{item.luggage}</span></div><p className="mt-2 font-semibold">{quote ? formatCurrency(quote.fare) : `from ${formatCurrency(item.minFare)}`}</p></div></button> })}</div></div> }
...
function Summary({ pickup, dropoff, quote, addOnsTotal, vehicle, pickupDate, pickupTime }: { pickup: PlaceSelection | null; dropoff: PlaceSelection | null; quote: ReturnType<typeof computeFare> | null; addOnsTotal: number; vehicle?: VehicleClass; pickupDate: string; pickupTime: string }) { const total = quote ? quote.fare + addOnsTotal : null; return <aside className="h-fit space-y-4 lg:sticky lg:top-24">{pickup && dropoff && <TripMap originLat={pickup.lat} originLng={pickup.lng} originLabel={pickup.address} originTime={pickupTime || undefined} destLat={dropoff.lat} destLng={dropoff.lng} destLabel={dropoff.address} /> }<div className="rounded-2xl border border-border/70 bg-card p-5"><h3 className="font-semibold">Trip summary</h3><div className="mt-4 space-y-3 text-sm"><Line label="Pickup" value={pickup?.address || "—"} /><Line label="Drop-off" value={dropoff?.address || "—"} /><Line label="When" value={pickupDate ? `${formatDate(pickupDate)} · ${pickupTime}` : "—"} /><Line label="Vehicle" value={vehicle?.name || "—"} />{addOnsTotal > 0 && <Line label="Add-ons" value={formatCurrency(addOnsTotal)} />}</div><div className="mt-5 border-t pt-4"><div className="flex items-end justify-between"><span className="text-sm text-muted-foreground">Total fare</span><span className="text-2xl font-semibold">{total != null ? formatCurrency(total) : "—"}</span></div></div></div></aside> }
