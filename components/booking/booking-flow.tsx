...
import { applyPromotion, computeDiscount, computeFare, congestionChargeFor, formatCurrency } from "@/lib/fleet"
...
const airportFromParams = (params: URLSearchParams): PlaceSelection | null => {
  const address = params.get("airportAddress")
  const lat = Number(params.get("airportLat"))
  const lng = Number(params.get("airportLng"))
  return address && Number.isFinite(lat) && Number.isFinite(lng) ? { placeId: params.get("airportId") || "", address, lat, lng } : null
}
...
export function BookingFlow({ vehicles = [], addOns = [], promotion = NO_PROMOTION, returnDiscount = NO_RETURN_DISCOUNT, stopPricing = NO_STOP_PRICING, congestionZones = NO_CONGESTION_ZONES }: { vehicles: VehicleClass[]; addOns: BookingAddOn[]; promotion?: SitePromotion; returnDiscount?: ReturnTripDiscount; stopPricing?: StopPricing; congestionZones?: CongestionZone[] }) {
  ...
  const initialPickup = direction === "from-airport" ? airport : exactPickup
  const initialDropoff = direction === "to-airport" ? airport : exactDropoff
  ...
  function applyPromo() {
    ...
    startPromoTransition(async () => {
      const res = await previewPromoCode(promoInput, vehicleFare + addOnsTotal + stopsTotal + congestionCharge)
...
function VehicleStep({ vehicleId, setVehicleId, distanceMiles, durationMinutes, vehicles, loading, promotion, congestionCharge }: { vehicleId: string; setVehicleId: (value: string) => void; distanceMiles: number | null; durationMinutes: number | null; vehicles: VehicleClass[]; loading: boolean; promotion: SitePromotion; congestionCharge: number }) {
  ...
  const priceOf = (item: VehicleClass) => {
    ...
    const vehicleFare = quote ? quote.fare : item.minFare
    // Promotion discounts the vehicle fare only — congestionCharge is added after, same as Summary/mobileTotal.
    const discountedFare = promotion.active ? applyPromotion(vehicleFare, promotion.discountPercent) : vehicleFare
    return { quoted: quote !== null, original: vehicleFare + congestionCharge, price: discountedFare + congestionCharge }
  }
