...
export function AirportQuoteActions({ page, onDarkBackground = false }: { page: AirportPagePresentation; onDarkBackground?: boolean }) {
  ...
  return <div className="mt-7 flex flex-col items-center justify-center gap-3">
    ...
    <p className={onDarkBackground ? "text-sm text-background/80" : "text-sm text-muted-foreground"}>Need help or having trouble with the quote service? <Link className="font-medium underline" href="/contact">Contact us</Link>.</p>
  </div>
}
